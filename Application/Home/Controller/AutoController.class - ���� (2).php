...
echo "====符合条件买入{$result['sym1']}=======\n\n";exit;
    ...
    */
    ...
    public function auto_buy3($sym){
      ...
      while(true){
        ...
        if($result['status'] == 1){
          ...
          $buy = $this->ratio2_buy($kline['data'][1]['close'],$close_avg * 1.005,$kline['data'][2]['close'],$result,$status);
          if($buy == 1){
            $status  = 0;
            $status2 = 1;
            $type    = 1;
        ...
        //卖出操作
        $sell = $this->ratio2_sell($stopless_price,$kline['data'][1]['close'],$close_avg * 0.995,$kline['data'][2]['close'],$status2,$type,$result);
    ...
    //策略2 买单操作
    public function ratio2_buy($up1_close,$close_avg,$up2_close,$result,$status){
        if($status == 0){
          echo "====status为0==不允许执行买入操作===\n\n";
          return 2;
        }
        if($up1_close > $close_avg && $up2_close > $close_avg){
            echo "====符合买入条件===\n\n";
            $this->buy_order($result['sym1'],$result['sym2'],$result['order_num'],$result['once_num'],$result['minunitmoney'],$result['maxunitmoney'],$result['id'],$result['uid'],$result['ratio']);
    ...
    //策略2  卖单操作
    public function ratio2_sell($stopless_price,$up1_close,$close_avg,$up2_close,$status2,$type,$result){
      if(($stopless_price == 1) || ($up1_close < $close_avg && $up2_close < $close_avg)){
          if($status2 == 1){
              if($type == 1){   //已经执行过买单   
                $where2['ratio_type'] = 2;
                $where2['status']     = 1;
