...
class CrontabController extends HomeController
{
    ...
    public function __construct($uid) {
        $this->accountId = $this->getAccountId($uid);
        if($this->accountId<0) {
            exit;
        }
    }
...
//        $result =  json_decode( json_encode( $result),true);
     ...
     * 查询Pro站指定账户的余额
     * [
     * 'usdt'=>["trade"=>438.08, "frozen"=>32.08],
     * 'ht'=>["trade"=>438.08, "frozen"=>32.08],
     * ]
     */
    public function getAccountIdBalance()
    {
        ...
        if($result['status']!='ok') {
            printf("get account balance error:%s\n", $result['err-msg']);
            return null;
        }
     ...
     * Pro站下单
     */
    public function getPlace($amount = '0.1', $symbol = 'btcusdt', $type = 'buy-limit', $price = '10000.1', $source = '',$uid)
    {
        $account_id = $this->accountId;
        $req = new req($uid);
        $result = $req->place_order((string)$account_id, $amount, $price, $symbol, $type);
        return $result;
    }
    ...
    public function getOrder($order_id)
    {
        ...
        if($result['status']!='ok') {
            printf("get order detail error:%s\n", $result['err-msg']);
        }
        return $result;
    }
...
//        $end_date = date('Y-m-d', time());\
        ...
        $size = '';
        $req = new req();
        $result = $req->get_order_orders($symbol,$types,$start_date,$end_date,$states,$from,$direct,$size);
        var_dump($result);
        exit();
    }
